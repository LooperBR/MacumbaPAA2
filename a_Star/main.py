...
class Graph:
    ...
    def dijkstra(self,raiz,termino):
        ...
        while(vertice_atual!=termino):
            ...
            for i in self.vertices[vertice_atual].neighbors:
                #se a distancia do vertice atual até o vizinho mais a heuristica for menor do que a distancia atual do vizinho atualiza a distancia do vizinho
                if(self.vertices[i[0]].dist==-1 or self.vertices[i[0]].dist > self.vertices[vertice_atual].dist+i[1]+self.heuristica(i[0],termino)):

                    self.vertices[i[0]].dist = self.vertices[vertice_atual].dist+i[1]+self.heuristica(i[0],termino)
    ...
    #define a função da heuristica
    def heuristica(self,origem,final):
        return sqrt(pow(self.vertices[origem].x-self.vertices[final].x,2)+pow(self.vertices[origem].y-self.vertices[final].y,2))
